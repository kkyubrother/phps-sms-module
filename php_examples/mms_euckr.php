...
class mms {
	 ...
	 * @return
	 */
	public function __construct($encoding='EUC-KR') {
		$this->server_url = "http://sms.phps.kr/lib/sendmms.sms";
		$this->cut = 5000;
		$this->params = array();
	...
	public function view($params = null) {
		...
		return $this->post(array('adminuser'=>$params['TR_ID'],
	...
	public function send($params = null) {
		...
		if (empty($params['TR_COMMENT'])) { $params['TR_COMMENT'] = ""; }
		if (empty($params['TR_SUBJECT'])) { $params['TR_SUBJECT'] = ""; }
		if (empty($params['TR_FILES'][0])) { $params['TR_FILES'][0] = ""; }
		if (empty($params['TR_FILES'][1])) { $params['TR_FILES'][1] = ""; }
		if (empty($params['TR_FILES'][2])) { $params['TR_FILES'][2] = ""; }
		if (empty($params['TR_FILES'][3])) { $params['TR_FILES'][3] = ""; }
		if (empty($params['TR_FILES'][4])) { $params['TR_FILES'][4] = ""; }
		if (empty($params['TR_FILES'][5])) { $params['TR_FILES'][5] = ""; }
		...
		if (strtoupper($this->encoding) == 'UTF-8') {
			if (function_exists('mb_convert_encoding')) {
				...
				$params['TR_SUBJECT'] = mb_convert_encoding($params['TR_SUBJECT'],'EUC-KR','UTF-8');
			} else if (function_exists('iconv')) {
				$params['TR_COMMENT'] = iconv('UTF-8','EUC-KR',$params['TR_COMMENT']);
				$params['TR_TXTMSG'] = iconv('UTF-8','EUC-KR',$params['TR_TXTMSG']);
				$params['TR_SUBJECT'] = iconv('UTF-8','EUC-KR',$params['TR_SUBJECT']);
			} else {
		...
		foreach ($group as $key => $pdata) {
			...
			$post = array('adminuser'=>$params['TR_ID'],
				...
				'subject'=>$params['TR_SUBJECT'],
				'msg'=>$params['TR_COMMENT'],
				'sms'=>$params['TR_TXTMSG'],
				'date'=>$params['TR_DATE'],
				'files[0]'=>$params['TR_FILES'][0],
				'files[1]'=>$params['TR_FILES'][1],
				'files[2]'=>$params['TR_FILES'][2],
				'files[3]'=>$params['TR_FILES'][3],
				'files[4]'=>$params['TR_FILES'][4],
				'files[5]'=>$params['TR_FILES'][5],
	...
	public function cancel($params = null) {
		...
		if (empty($params['TR_ID'])) { return array('msg'=>'TR_ID is empty','status'=>'fail'); }
		if (empty($params['TR_KEY'])) { return array('msg'=>'TR_KEY is empty','status'=>'fail'); }
		if (empty($params['TR_NUM'])) { return array('msg'=>'TR_NUM is empty','status'=>'fail'); }
